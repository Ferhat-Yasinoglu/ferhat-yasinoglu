// Kısa bildirimler (toast): aria-live ile ekran okuyucuya da düşer.
import { simge } from './simge.js';

const SIMGE = { bilgi: 'bilgi', basari: 'basari', hata: 'hata', uyari: 'uyari' };
let kap = null;

function kapAl() {
  if (!kap || !kap.isConnected) {
...
export function bildir(metin, { tur = 'bilgi', sure = 3500, eylem } = {}) {
  ...
  e.className = `bildirim bildirim--${tur}`;
  ...
  if (eylem) {
    ...
    b.type = 'button'; b.className = 'btn btn--kucuk'; b.textContent = eylem.metin;
    b.onclick = () => { eylem.cb(); e.remove(); };
    e.appendChild(b);
  }
  k.appendChild(e);
...
export const uyar = (m, s) => bildir(m, { tur: 'uyari', sure: 5000, ...s });
