...
const config = authFromEnv({ CHATPLACE_API_KEYS: "key-one, key-two" } as NodeJS.ProcessEnv);
...
describe("authenticate", () => {
  ...
  it("can be disabled explicitly even with keys present", () => {
    const open = authFromEnv({ CHATPLACE_API_KEYS: "key-one", CHATPLACE_AUTH_DISABLED: "1" } as NodeJS.ProcessEnv);
