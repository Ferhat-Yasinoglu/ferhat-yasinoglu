...
describe("resources", () => {
  const spec = assertSpec({
    ...base,
    resources: [
      { uri: "chatplace://channels", name: "Channels", mimeType: "application/json", text: '{"channels":[]}' },
      { uri: "chatplace://empty", name: "Empty" },
    ],
    resourceTemplates: [{ uriTemplate: "chatplace://bot/{bot_id}", name: "Bot" }],
  });
  ...
  it("lists resources without leaking inline content into the listing", async () => {
    ...
    expect(resources[0]?.uri).toBe("chatplace://channels");
    await client.close();
  });
  ...
  it("lists resource templates", async () => {
    ...
    expect(resourceTemplates[0]?.uriTemplate).toBe("chatplace://bot/{bot_id}");
    await client.close();
  });

  it("reads inline content back", async () => {
    const client = await connect(spec);
    const { contents } = await client.readResource({ uri: "chatplace://channels" });
  ...
  it("stubs a resource declared without content", async () => {
    const client = await connect(spec);
    const { contents } = await client.readResource({ uri: "chatplace://empty" });
  ...
  it("rejects an unknown uri", async () => {
    const client = await connect(spec);
    await expect(client.readResource({ uri: "chatplace://missing" })).rejects.toThrow(/Unknown resource/);
    await client.close();
  });
});
