...
import { el, btn, rozet, temizle, girdi, goreliZaman, sayfaBas } from '../cekirdek/dom.js';
...
export default {
  ...
  async cizim(kok, ctx) {
    ...
    kok.append(
      sayfaBas(t('nav.akislar', 'Akışlar'), { alt: t('akislar.alt', 'Gelen mesajlara otomatik cevap veren adım dizileri.'), eylemler: [btn('+ ' + t('akislar.yeni', 'Yeni akış'), { class: 'btn btn--birincil', onclick: () => git('/akislar/yeni') })] }),
    ...
    async function ciz() {
      ...
      for (const a of gorunen) {
        ...
        const onizleme = (a.adimlar || []).slice(0, 4).map((s) => ADIM_BILGI[s.type]?.simge || '•').join(' → ') + ((a.adimlar || []).length > 4 ? ' → …' : '');
        liste.appendChild(el('section', { class: 'kart kart--tik', onclick: () => git(`/akis/${a.id}`) },
          ...
          el('div', { class: 'satir satir--arasi', onclick: (e) => e.stopPropagation() },
            ...
            el('div', { class: 'satir' }, btn(t('akis.test', 'Test'), { class: 'btn btn--kucuk', onclick: () => git(`/akis/${a.id}/test`) }), btn('⋯', { class: 'btn btn--kucuk btn--ikon btn--sade', 'aria-label': t('genel.daha', 'Daha'), onclick: (e) => menu(e.currentTarget, a) })))));
      }
    ...
    function menu(hedef, a) {
      ...
      const m = el('div', { class: 'acilir__menu' },
        ...
        btn('⬇ ' + t('akis.disa', 'JSON indir'), { onclick: () => { const blob = new Blob([JSON.stringify({ akis: a }, null, 2)], { type: 'application/json' }); const u = URL.createObjectURL(blob); const l = document.createElement('a'); l.href = u; l.download = `${a.ad}.json`; l.click(); m.remove(); } }),
        btn('🗑 ' + t('akis.sil', 'Sil'), { style: { color: 'rgb(var(--kirmizi))' }, onclick: async () => { m.remove(); if (await ctx.onayla(t('akis.sil_onay', '"{ad}" silinsin mi?', { ad: a.ad }), { tehlikeli: true })) { await depo.sil('akislar', a.id); for (const x of await depo.listele('tetikleyiciler', { filtre: { akis_id: a.id } })) await depo.sil('tetikleyiciler', x.id); ciz(); } } }));
