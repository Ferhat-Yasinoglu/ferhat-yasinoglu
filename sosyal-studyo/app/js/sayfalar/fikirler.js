...
import { simge, simgeDolu } from '../cekirdek/simge.js';
...
export default {
  ...
  async cizim(kok, ctx) {
    ...
    const uretBtn = btnS('parilti', t('fikir.uret', '10 fikir üret'), { class: 'btn btn--birincil', disabled: !mevcut, title: mevcut ? '' : t('ai.yerel', 'AI için Worker\'ı bağla'), onclick: async () => { try { uretBtn.disabled = true; const r = await ai.iste('fikir_uret', { nis: nis.value, hedef: hedef.value, format: format.value }); for (const f of r.fikirler || []) await depo.kaydet('fikirler', { ...f, format: f.format || format.value, hedef: hedef.value, durum: 'fikir', kaynak: 'ai' }); ctx.basari(t('fikir.uretildi', '{n} fikir eklendi', { n: (r.fikirler || []).length })); ciz(); } catch (e) { ctx.hata(e.message); } finally { uretBtn.disabled = !mevcut; } } });
    const liste = el('div', { class: 'izgara' });
    async function ciz() {
      temizle(liste);
      const fikirler = await depo.listele('fikirler', { sirala: 'guncellendi', azalan: true });
      for (const f of fikirler) liste.appendChild(kart(el('div', { class: 'satir satir--arasi' }, el('h2', { class: 'kart__baslik' }, f.baslik), rozet(f.durum, { fikir: 'gri', senaryo: 'mavi', uretildi: 'altin', yayinlandi: 'yesil' }[f.durum] || 'gri')), el('p', { class: 'kart__alt' }, f.aciklama), el('div', { class: 'satir' }, rozet(f.format || '—', 'mor'), f.demo ? rozet('Demo', 'gri') : null, f.favori ? rozet(t('fikir.favori', 'Favori'), 'altin') : null), el('div', { class: 'satir' }, btn(t('fikir.senaryo', 'Senaryo yaz'), { class: 'btn btn--kucuk btn--birincil', onclick: () => senaryoYaz(f) }), btn(f.favori ? simgeDolu('yildiz') : simge('yildiz'), { class: 'btn btn--kucuk btn--ikon', onclick: async () => { await depo.kaydet('fikirler', { ...f, favori: !f.favori }); ciz(); } }), btn(simge('kapat'), { class: 'btn btn--kucuk btn--ikon', onclick: async () => { await depo.sil('fikirler', f.id); ciz(); } }))));
    ...
    async function senaryoYaz(f) {
      ...
      const aiBtn = btnS('parilti', t('senaryo.ai', 'AI ile yaz'), { class: 'btn btn--kucuk', disabled: !mevcut, onclick: async () => { try { const r = await ai.iste('senaryo_yaz', { baslik: f.baslik, aciklama: f.aciklama, format: f.format }); kanca.value = r.kanca || ''; sahneler.value = (r.sahneler || []).map((s) => `${s.sure_sn}s | ${s.metin}`).join('\n'); cta.value = r.cta || ''; hesapla(); } catch (e) { ctx.hata(e.message); } } });
      const govde = el('div', {}, alan(t('senaryo.kanca_etiket', 'Kanca'), kanca), alan(t('senaryo.sahneler', 'Sahneler (saniye | metin)'), sahneler), alan('CTA', cta), sure, el('div', { class: 'satir' }, aiBtn, btnS('kanca', t('senaryo.kancaya', 'Kancayı kütüphaneye ekle'), { class: 'btn btn--kucuk', onclick: async () => { if (kanca.value.trim()) { await depo.kaydet('kancalar', { metin: kanca.value.trim(), dil: 'tr', format: f.format, nis: [], kaynak: 'senaryo' }); ctx.basari(t('genel.eklendi', 'Eklendi')); } } })));
