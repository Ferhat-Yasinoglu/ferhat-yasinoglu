...
async function sohbetCiz(kok, ctx, id) {
  ...
  const yaz = el('form', { class: 'satir', onsubmit: (e) => { e.preventDefault(); ctx.bildir(t('sohbet.yerel_gonderim', 'Gönderim bağlı modda çalışır; yerel modda sohbetler yalnız okunur.')); } }, girdi({ placeholder: sanal ? t('sohbet.sanal_yaz', 'Simülatörde konuşmak için akışın Test sekmesini kullan.') : t('sohbet.yaz', 'Mesaj yaz…'), disabled: sanal }), btn('➤', { type: 'submit', class: 'btn btn--birincil btn--ikon', disabled: sanal }));
