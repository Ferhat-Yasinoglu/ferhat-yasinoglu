...
export default {
  ...
  async cizim(kok, ctx) {
    ...
    function bilgiCiz() { temizle(bilgiKap); for (const [i, b] of (brif.bilgi_tabani || []).entries()) bilgiKap.appendChild(el('div', { class: 'liste__satir' }, el('div', { class: 'liste__govde' }, el('div', { class: 'liste__baslik' }, b.baslik), el('div', { class: 'liste__alt' }, b.metin.slice(0, 120))), el('input', { type: 'checkbox', checked: b.aktif !== 0, title: 'Açık/kapalı', onchange: (e) => { b.aktif = e.target.checked ? 1 : 0; kaydet(); } }), btn(simge('kalem'), { class: 'btn btn--kucuk btn--ikon', onclick: () => bilgiDuzenle(i) }), btn(simge('kapat'), { class: 'btn btn--kucuk btn--ikon', onclick: () => { brif.bilgi_tabani.splice(i, 1); kaydet(); bilgiCiz(); } }))); }
