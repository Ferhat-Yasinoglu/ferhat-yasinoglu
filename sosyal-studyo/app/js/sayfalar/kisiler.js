...
import { el, btn, kart, rozet, temizle, girdi, secim, alan, goreliZaman, sayfaBas, btnS } from '../cekirdek/dom.js';
  ...
  async cizim(kok, ctx) {
    ...
    async function kisilerCiz() {
      ...
      async function listele() {
        ...
        if (!kisiler.length) { liste.appendChild(bos({ simge: 'kisiler', baslik: t('kisiler.bos', 'Kişi yok'), aciklama: t('kisiler.bos_aciklama', 'Botuna ilk mesaj gelince burada görünür; yerel modda elle ekleyebilir ya da CSV alabilirsin.') })); return; }
        for (const k of kisiler) {
          const cb = el('label', { class: 'onay-hucre' }, el('input', { type: 'checkbox', 'aria-label': t('kisiler.sec', 'Seç'), onchange: (e) => { e.target.checked ? secili.add(k.id) : secili.delete(k.id); topluBar.hidden = secili.size === 0; } }));
          liste.appendChild(el('div', { class: 'liste__satir' }, cb, el('div', { class: 'avatar' }, (k.ad || '?').slice(0, 1).toUpperCase()), el('a', { class: 'liste__govde', href: `#/kisi/${k.id}`, style: { textDecoration: 'none', color: 'inherit' } }, el('div', { class: 'liste__baslik' }, `${k.ad || k.kullanici_adi || k.dis_id}${k.demo ? ' · demo' : ''}`), el('div', { class: 'liste__alt' }, `${k.kanal} · ${(k.etiketler || []).join(', ') || '—'} · ${k.puan || 0} ${t('kisi.puan', 'puan')}`)), el('div', { style: { textAlign: 'end' } }, el('div', { class: 'kart__alt' }, goreliZaman(k.son_gelen, t)), pencereAcik(k) ? rozet(t('sohbet.acik', 'Açık'), 'yesil') : rozet(t('sohbet.kapali', 'Kapalı'), 'gri'))));
        }
      }
    ...
    async function etiketlerCiz() {
      ...
      for (const e of etiketler) {
        ...
        liste.appendChild(el('div', { class: 'liste__satir' }, rozet(e.ad, e.renk || 'gri'), el('div', { class: 'liste__govde' }, el('div', { class: 'liste__alt' }, `${n} ${t('kisiler.kisi', 'kişi')}${e.sistem ? ' · ' + t('kisiler.sistem', 'sistem') : ''}`)),
          btn(simge('kalem'), { class: 'btn btn--kucuk btn--ikon', disabled: !!e.sistem, onclick: async () => { const yeni = await sor(t('kisiler.yeniden_adlandir', 'Yeni ad'), { varsayilan: e.ad }); if (!yeni || yeni === e.ad) return; await depo.kaydet('etiketler', { ...e, ad: yeni }); for (const k of kisiler) if ((k.etiketler || []).includes(e.ad)) await depo.kaydet('kisiler', { ...k, etiketler: k.etiketler.map((x) => (x === e.ad ? yeni : x)) }); etiketlerCiz(); } }),
          btn(simge('kapat'), { class: 'btn btn--kucuk btn--ikon', disabled: !!e.sistem, onclick: async () => { if (await onayla(t('kisiler.etiket_sil', '"{ad}" etiketi {n} kişiden kaldırılacak.', { ad: e.ad, n }), { tehlikeli: true })) { await depo.sil('etiketler', e.id); for (const k of kisiler) if ((k.etiketler || []).includes(e.ad)) await depo.kaydet('kisiler', { ...k, etiketler: k.etiketler.filter((x) => x !== e.ad) }); etiketlerCiz(); } } })));
      }
      govde.append(btnS('arti', t('kisiler.etiket_yeni', 'Etiket'), { class: 'btn btn--birincil btn--kucuk', onclick: async () => { const ad = await sor(t('kisiler.etiket_adi', 'Etiket adı')); if (ad) { await depo.kaydet('etiketler', { ad: ad.trim(), renk: ['altin', 'mavi', 'yesil', 'mor', 'sari'][etiketler.length % 5] }); etiketlerCiz(); } } }), el('div', { style: { height: '8px' } }), liste);
    }
    async function puanlarCiz() {
      ...
      const tablo = el('table', { class: 'tablo' }, el('thead', {}, el('tr', {}, el('th', {}, t('puan.olay', 'Olay')), el('th', {}, t('puan.puan', 'Puan')), el('th', {}, t('puan.tavan', 'Günlük tavan')), el('th', {}, ''))),
        el('tbody', {}, ...kurallar.map((k) => el('tr', {}, el('td', {}, k.olay), el('td', {}, girdi({ type: 'number', value: k.puan, style: { width: '80px' }, onchange: (e) => depo.kaydet('puan_kurallari', { ...k, puan: Number(e.target.value) }) })), el('td', {}, girdi({ type: 'number', value: k.gunlukTavan, style: { width: '80px' }, onchange: (e) => depo.kaydet('puan_kurallari', { ...k, gunlukTavan: Number(e.target.value) }) })), el('td', {}, el('label', { class: 'onay-hucre' }, el('input', { type: 'checkbox', checked: !!k.aktif, 'aria-label': t('kisiler.kural_aktif', 'Kural aktif'), onchange: (e) => depo.kaydet('puan_kurallari', { ...k, aktif: e.target.checked ? 1 : 0 }) })))))));
      const liderlik = el('ol', { class: 'liste', style: { padding: 0, margin: 0, listStyle: 'none' } }, ...kisiler.map((k, i) => el('li', { class: 'liste__satir' }, el('span', { class: 'avatar' }, String(i + 1)), el('div', { class: 'liste__govde' }, el('div', { class: 'liste__baslik' }, k.ad || k.kullanici_adi), el('div', { class: 'liste__alt' }, k.kanal)), el('strong', { class: 'satir' }, simge('yildiz', { boy: 14 }), String(k.puan || 0)))));
      const png = btnS('galeri', t('puan.png', 'Liderlik tablosunu PNG indir'), { class: 'btn btn--kucuk', onclick: () => liderlikPng(kisiler.slice(0, 10)) });
