...
export default {
  ...
  async cizim(kok, ctx) {
    ...
    function etiketleriCiz(kisi) { temizle(etiketKap); for (const e of kisi.etiketler || []) etiketKap.appendChild(el('span', { class: 'cip' }, e, btn('✕', { onclick: async () => { const y = await depo.kaydet('kisiler', { ...kisi, etiketler: kisi.etiketler.filter((x) => x !== e) }); etiketleriCiz(y); } }))); etiketKap.appendChild(btn('+', { class: 'btn btn--kucuk', onclick: async () => { const e = await sor(t('kisiler.etiket_adi', 'Etiket adı')); if (e && !(kisi.etiketler || []).includes(e)) { const y = await depo.kaydet('kisiler', { ...kisi, etiketler: [...(kisi.etiketler || []), e] }); etiketleriCiz(y); } } })); }
    etiketleriCiz(k);
    ...
    kok.append(
      ...
      kart(el('h2', { class: 'kart__baslik' }, `⭐ ${t('kisi.puan', 'Puan')}: ${k.puan || 0}`), el('div', { class: 'satir' }, btn('+5', { class: 'btn btn--kucuk', onclick: () => puanEkle(5) }), btn('−5', { class: 'btn btn--kucuk', onclick: () => puanEkle(-5) })), ...puanlar.slice(0, 10).map((p) => el('div', { class: 'kart__alt' }, `${p.delta > 0 ? '+' : ''}${p.delta} · ${p.neden} · ${goreliZaman(p.olusturuldu, t)}`))),
    ...
    async function puanEkle(delta) { const y = await depo.kaydet('kisiler', { ...(await depo.al('kisiler', k.id)), puan: (k.puan || 0) + delta }); await depo.kaydet('puan_olaylari', { kisi_id: k.id, delta, neden: 'elle', kaynak: 'manuel', olay_anahtari: 'manuel:' + Date.now() }); k.puan = y.puan; ctx.basari(`⭐ ${y.puan}`); ctx.git(`/kisi/${k.id}`); }
  },
};
