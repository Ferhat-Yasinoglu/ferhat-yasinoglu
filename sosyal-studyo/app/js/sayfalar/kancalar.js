...
import { el, btn, kart, rozet, temizle, girdi, secim, sayfaBas, btnS } from '../cekirdek/dom.js';
  ...
  async cizim(kok, ctx) {
    ...
    kok.append(sayfaBas(t('nav.kancalar', 'Kanca Kütüphanesi'), { alt: t('kancalar.alt', 'İlk üç saniyeyi kurtaran açılış cümleleri.') }), el('div', { class: 'satir', style: { marginBlockEnd: '16px' } }, arama, dil, format),
      el('div', { class: 'satir', style: { margin: '8px 0' } },
        btnS('arti', t('kanca.yaz', 'Kanca yaz'), { class: 'btn btn--birincil btn--kucuk', onclick: async () => { const m = await sor(t('kanca.yaz', 'Kanca yaz'), { cokSatir: true }); if (!m) return; const var_ = (await depo.listele('kancalar')).find((k) => normalize(k.metin) === normalize(m)); if (var_) { ctx.bildir(t('kanca.zaten', 'Bu kanca zaten var; favoriye eklendi.')); await depo.kaydet('kancalar', { ...var_, favori: true }); } else await depo.kaydet('kancalar', { metin: m, dil: 'tr', format: 'reels', nis: [], kaynak: 'elle' }); ciz(); } }),
        btnS('kutu', t('kanca.hazir', 'Hazır 30 kancayı yükle'), { class: 'btn btn--kucuk', onclick: async () => { const r = await fetch('./data/kancalar-hazir.json'); const liste = await r.json(); const mevcutlar = new Set((await depo.listele('kancalar')).map((k) => normalize(k.metin))); let n = 0; for (const k of liste) if (!mevcutlar.has(normalize(k.metin))) { await depo.kaydet('kancalar', { ...k, kaynak: 'tohum' }); n++; } ctx.basari(t('kanca.yuklendi', '{n} kanca eklendi', { n })); ciz(); } }),
        btnS('parilti', t('kanca.ai', 'AI ile 10 kanca öner'), { class: 'btn btn--kucuk', disabled: !mevcut, onclick: async () => { const konu = await sor(t('kanca.konu', 'Konu / niş')); if (!konu) return; try { const r = await ai.iste('kanca_oner', { konu, dil: 'tr' }); for (const m of r.kancalar || []) await depo.kaydet('kancalar', { metin: m, dil: 'tr', format: 'reels', nis: [konu], kaynak: 'ai' }); ciz(); } catch (e) { ctx.hata(e.message); } } }),
        btnS('indir', 'JSON', { class: 'btn btn--kucuk', onclick: async () => { const b = new Blob([JSON.stringify(await depo.listele('kancalar'), null, 2)], { type: 'application/json' }); const u = URL.createObjectURL(b); const l = document.createElement('a'); l.href = u; l.download = 'kancalar.json'; l.click(); } })),
      liste);
    ciz();
  },
};
