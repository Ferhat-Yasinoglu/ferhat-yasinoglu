...
import { indir, paylas, dosyadanOku, hatirlatmaGerekli, hatirlatmaMetni } from '../depo/yedek.js';
...
export default {
  ...
  async cizim(kok, ctx) {
    ...
    async function kanallarCiz() {
      ...
      const ekle = el('div', { class: 'izgara izgara--dar' }, ...Object.entries(KANALLAR).map(([k, b]) => kart(el('div', { class: 'satir satir--arasi' }, el('span', { class: 'kart__baslik' }, simge(b.ikon), b.ad), rozet(b.etiket.ad, b.etiket.renk)), el('p', { class: 'kart__alt' }, b.ozet), btn(b.kurulum_dk ? `${t('kanal.kur', 'Kur')} · ~${b.kurulum_dk} dk` : t('kanal.bilgi', 'Bilgi'), { class: 'btn btn--kucuk', onclick: () => git(`/ayarlar/kurulum/${k}`) }))));
    ...
    async function workerCiz() {
      ...
      async function dogrula() {
        ...
        try {
          ...
          sonuc.replaceChildren(kart(el('h2', { class: 'kart__baslik' }, simge('onay'), '' + t('worker.bagli', 'Bağlandı')), el('p', { class: 'kart__alt' }, `${t('ayar.surum', 'Sürüm')} ${j.surum || '?'} · ${t('ayar.sema', 'şema')} ${j.sema} · ${j.prova ? 'PROVA' : 'CANLI'} · AI: ${j.saglayici || 'yok'}`), ...((dj.veri?.doktor || []).map((x) => el('div', { class: 'kart__alt' }, `${x.ad}: ${x.detay || ''}`)))));
    ...
    async function yedekCiz() {
      ...
      govde.append(h.gerekli ? el('div', { class: 'bant bant--sari' }, simge('kaydet'), hatirlatmaMetni(h, t)) : el('div', { class: 'bant bant--mavi' }, simge('onay'), '' + t('yedek.guncel', 'Yedek güncel')),
        kart(el('h2', { class: 'kart__baslik' }, t('yedek.al', 'Yedek al')), el('p', { class: 'kart__alt' }, `${t('yedek.son', 'Son yedek')}: ${meta.son_yedek ? goreliZaman(meta.son_yedek, t) : '—'} · ${meta.degisiklik_sayaci || 0} ${t('yedek.degisiklik', 'yedeklenmemiş değişiklik')}`), el('p', { class: 'kart__alt' }, t('yedek.aciklama', 'JSON dosyası: akışlar, kişiler, kurallar, içerik. Gizli anahtar ve mesaj içeriği girmez (isteğe bağlı).')), el('div', { class: 'satir' }, btnS('indir', t('bant.yedek_indir', 'Yedeği indir'), { class: 'btn btn--birincil', onclick: async () => { await indir(await depo.disaAktar({ gunlukDahil: gunlukCb.checked })); ctx.basari(t('yedek.indirildi', 'Yedek indirildi')); ctx.yenileBantlar(); yedekCiz(); } }), btnS('disari', t('yedek.paylas', 'Paylaş (Drive, Dosyalar…)'), { onclick: async () => { const ok = await paylas(await depo.disaAktar({ gunlukDahil: gunlukCb.checked })); if (!ok) ctx.bildir(t('yedek.paylas_yok', 'Bu cihazda dosya paylaşımı yok; indir.')); else { ctx.yenileBantlar(); yedekCiz(); } } })), el('label', { class: 'cip' }, (function () { return gunlukCb; })(), ' ' + t('yedek.gunluk_dahil', 'günlük ve mesajlar dahil'))),
        kart(el('h2', { class: 'kart__baslik' }, t('yedek.geri', 'Geri yükle')), el('label', { class: 'btn' }, simge('yukle'), '' + t('yedek.dosya_sec', 'Yedek dosyası seç'), dosya), onizleme),
