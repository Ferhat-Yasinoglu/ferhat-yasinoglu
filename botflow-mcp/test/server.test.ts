...
describe("tools/call", () => {
  ...
  it("rejects arguments that violate the input schema", async () => {
    const client = await connect();
    // list_flows requires bot_id.
    const result = await client.callTool({ name: "list_flows", arguments: {} });

    expect(result.isError).toBe(true);
    expect(textOf(result)).toContain("bot_id");
    await client.close();
  });
  ...
  it("rejects a value of the wrong type", async () => {
    ...
    const result = await client.callTool({
      ...
      arguments: { bot_id: "bot_1", limit: "twenty" },
    });
  ...
  it("stubs a valid call with content matching the output schema", async () => {
    const client = await connect();
    // No handlers are registered in this suite, so the stub answers instead.
    const result = await client.callTool({ name: "list_bots", arguments: {} });

    expect(result.isError).toBeFalsy();
    const structured = result.structuredContent as { bots?: unknown[] } | undefined;
    expect(Array.isArray(structured?.bots)).toBe(true);
    await client.close();
  });

  it("applies schema defaults before handing arguments to the handler", async () => {
    const client = await connect();
    const result = await client.callTool({ name: "list_subscribers", arguments: { bot_id: "bot_1" } });

    // The stub echoes what it received; limit has a default of 50.
    expect(textOf(result)).toContain('"limit": 50');
    await client.close();
  });
});
