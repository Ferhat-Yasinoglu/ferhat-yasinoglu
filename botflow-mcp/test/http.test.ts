...
beforeAll(async () => {
  server = await startHttpServer({
    spec: loadSpec(),
    auth: authFromEnv({ BOTFLOW_API_KEYS: API_KEY } as NodeJS.ProcessEnv),
    port: 0,
    host: "127.0.0.1",
  });
...
describe("streamable http endpoint", () => {
  it("completes the handshake and lists tools with a valid key", async () => {
    ...
    expect(tools.map((t) => t.name)).toContain("list_bots");
  ...
  it("reports server info from the spec", async () => {
    ...
    expect(client.getServerVersion()?.name).toBe("botflow-mcp");
  ...
  it("calls a tool end to end", async () => {
    ...
    expect(result.structuredContent).toHaveProperty("bots");
