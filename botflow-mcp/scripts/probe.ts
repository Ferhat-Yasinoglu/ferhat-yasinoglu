 ...
 *
 *   npm run probe -- --url https://mcp.chatplace.io/mcp --key "$BOTFLOW_KEY"
 *
...
async function main(argv: string[]): Promise<void> {
  ...
  const client = new Client({ name: "botflow-mcp-probe", version: "0.1.0" });
  ...
  const spec = specFromToolsList(surface, {
    ...
    serverInfo: serverInfo
      ...
      : { name: "botflow-mcp", version: "0.1.0" },
