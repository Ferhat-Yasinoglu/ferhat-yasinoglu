...
export type SpecSource = {
   ...
   *
   * `authored`    — hand-written for this product; the normal case here.
   * `imported`    — read off a live server with `npm run probe`, matching it
   *                 one-to-one. Used when cloning someone else's surface.
   * `placeholder` — a guess at some other server's shape, with no live
   *                 implementation behind it. Reported loudly so it is never
   *                 mistaken for the real thing.
   */
  origin: "authored" | "imported" | "placeholder";
  server?: string;
  importedAt?: string;
